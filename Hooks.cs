...
namespace LargerInventory
{
    internal class Hooks
    {
        ...
        internal static void Load()
        {
            ...
            On.Terraria.Recipe.FindRecipes += FindRecipes_Replace;
            On.Terraria.Main.CraftItem += CraftItem_Replace;
            //On.Terraria.GameContent.ChromaHotkeyPainter.Step_QuickHeal += Catch_ChromaHotkeyPainter_Step_QuickHeal_Called;
            On.Terraria.Player.QuickHeal_GetItemToUse += QuickHeal_GetItemToUse_Replace;
            On.Terraria.Player.QuickHeal += QuickHeal_Replace;
            On.Terraria.Player.QuickMana_GetItemToUse += QuickMana_GetItemToUse_Replace;
            On.Terraria.Player.QuickMana += QuickMana_Replace;
        ...
        public static bool TryStackItem(Terraria.Item target, Terraria.Item pickItem, out bool HasSpaceLeft)
        {
            ...
            {
                if (target.stack == 0 || target.stack < target.maxStack) HasSpaceLeft = true;
            ...
            if (space > 0)
            {
                if (pickItem.IsACoin) Terraria.Audio.SoundEngine.PlaySound(SoundID.CoinPickup, pickItem.position);
                else Terraria.Audio.SoundEngine.PlaySound(SoundID.Grab, pickItem.position);
        ...
        {
            var player = self.GetModPlayer<PlayerManager>().inventoryPlayer;
            player.SaveInventory();
            bool isACoin = item.IsACoin;
            if (item.noGrabDelay > 0 || (item.uniqueStack && self.HasItem(item.type))) return item;
            //mod代码
            //不可堆叠物品直接贪心
            if (item.maxStack == 1 && !item.FitsAmmoSlot())
            {
                for (int j = 0; j < player.pages.Count; j++)
                    for (int k = 0; k < 50; k++)
                        if (player.pages[Wrap(0, player.pages.Count, player.NowInventoryIndex + j)].Inventory[k].IsAir)
                        {
                            if (player.NowInventoryIndex == Wrap(0, player.pages.Count, player.NowInventoryIndex + j)) self.inventory[k] = item;
                            else player.pages[Wrap(0, player.pages.Count, player.NowInventoryIndex + j)].Inventory[k] = item;
            ...
            //不想传参弄成员变量就直接lambda捕获了，这段代码重复了4遍直接写成个方法
            var func = (int from, int to, out Terraria.Item result) =>
            {
                for (int j = 0; j < player.pages.Count; j++)
                {
                    ...
                    {
                        var t = player.pages[Wrap(0, player.pages.Count, player.NowInventoryIndex - j - 1)].Inventory[k];
                        if (TryStackItem(t, item, out bool left) && !left) possibleStackPos.Remove(t.netID);
                        if (left) possibleStackPos[t.netID] = (k, Wrap(0, player.pages.Count, player.NowInventoryIndex - j - 1));
                        if (item.stack == 0)
                        {
                ...
                {
                    if (possibleStackPos[0].page == player.NowInventoryIndex) self.inventory[possibleStackPos[0].index] = item;
                    else player.pages[possibleStackPos[0].page].Inventory[possibleStackPos[0].index] = item;
                    result = new Terraria.Item();
                    if (item.IsACoin) Terraria.Audio.SoundEngine.PlaySound(SoundID.CoinPickup, item.position);
                    else Terraria.Audio.SoundEngine.PlaySound(SoundID.Grab, item.position);
            ...
            if (item.maxStack != 1 && possibleStackPos.TryGetValue(item.netID, out var value))
            {
                ...
                {
                    if (item.stack == 0) return new Terraria.Item();
                    else possibleStackPos.Remove(item.netID);
                }
            }
            if (item.FitsAmmoSlot() && func(57, 53, out var v1)) return v1;
            if(item.IsACoin)
            {
                //堆叠钱摆烂了
                if (func(53, 49, out var v2))
                {
                    self.DoCoins(53);
                    self.DoCoins(52);
                    self.DoCoins(51);
            ...
            if (func(49, -1, out var v3)) return v3;
            if (settings.CanGoIntoVoidVault && self.IsVoidVaultEnabled && MyCanVoidVaultAccept(item) && getItem_VoidVault.Invoke(self, plr, self.bank4.item, item, settings, item)) return new Terraria.Item();
            return item;
        }
        ...
        private static getItem_VoidVaultType getItem_VoidVault = typeof(Terraria.Player).GetMethod("GetItem_VoidVault", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).CreateDelegate<getItem_VoidVaultType>();
                    ...
                    foreach (Page p in Manager.Activing.pages)
                    {
                        for (int index = 0; index < 58; index++)
                        {
                            ...
                            else
                            {
                                if (index < 50)
                                {
                                    items.Add(p.Inventory[index].netID, new(new() { p.Inventory[index] }, new(), new()));
                                }
                                else if (index < 54)
                                {
                                    items.Add(p.Inventory[index].netID, new(new(), new() { p.Inventory[index] }, new()));
                                }
                                else
                                {
                                    items.Add(p.Inventory[index].netID, new(new(), new(), new() { p.Inventory[index] }));
                                }
                            }
                        }
                    }
                    for (int j = 0; j < Terraria.Main.item.Length; j++)
                    {
                        ...
                        if (!(!item.active || item.noGrabDelay != 0 || item.playerIndexTheItemIsReservedFor != i || !self.CanAcceptItemIntoInventory(item)))
                        {
                            if (item.stack > 0 && ItemLoader.CanPickup(item, self) && item.Hitbox.Intersects(self.Hitbox))
                            {
                                ...
                                else if(items.TryGetValue(0,out var voidslot))
                                {
                                    if (item.IsACoin)
                                    {
                                        list.AddRange(voidslot.Item2);
                                    }
                                    if (item.ammo > 0 && !item.notAmmo)
                                    {
                                        list.AddRange(voidslot.Item3);
                                    }
                                    list.AddRange(voidslot.Item1);
                                    for (int k = 0; k < list.Count; k++)
                                    {
